// src/components/EditRadiatorModal.jsx
...
const EditRadiatorModal = ({ isOpen, onClose, onSuccess, radiator }) => {
  ...
  // Populate form when radiator changes
  useEffect(() => {
    if (radiator) {
      setFormData({
        brand: radiator.brand || '',
        name: radiator.name || '',
        year: radiator.year || new Date().getFullYear()
      });
    }
  }, [radiator]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const result = await radiatorService.update(radiator.id, formData);
      ...
      <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div className="mt-3">
          ...
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-gray-900">Edit Radiator</h3>
            <button
              onClick={handleClose}
              disabled={loading}
          ...
          {/* Show current code (read-only) */}
          <div className="mb-4 p-3 bg-gray-50 rounded">
            <span className="text-sm text-gray-600">Code: </span>
            <span className="text-sm font-medium text-gray-900">{radiator.code}</span>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end space-x-3 pt-4">
              ...
              >
                {loading ? 'Updating...' : 'Update Radiator'}
              </button>
            </div>
          </form>
        </div>
