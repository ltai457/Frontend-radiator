...
const EditWarehouseModal = ({ isOpen, onClose, onSuccess, warehouse }) => {
  ...
  const updateField = (key, value) => {
    ...
    if (key === 'code') {
      if (value.trim().toUpperCase() === warehouse.code) {
        setCodeValidation({ isValid: true, message: 'Current code' });
      } else {
        validateCode(value);
      }
  ...
  const handleCodeBlur = () => {
    if (form.code.trim() && form.code.trim().toUpperCase() !== warehouse.code) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={`Edit Warehouse — ${warehouse.name}`}>
